...
qq.parseJson = function(json) {
    ...
    if (window.JSON && qq.isFunction(JSON.parse)) {
    	var bugData = json.split("}");
    	var fixedJson = '';
    	if(bugData.length > 0){
    		fixedJson = bugData[0];
    	}
    	else{
    		fixedJson = "{'success':false,'id':null,'status':'Internal Server Error'}";
    	}
        
        if(fixedJson.charAt(fixedJson.length-1) != "}"){
        	fixedJson += "}";
        }
            ...
            onComplete: function(id, name, result, xhr){
            	if($.isEmptyObject(result)){
            		if(typeof xhr != 'undefined'){
            			result = JSON.parse(xhr.responseText.split("}")[0]+"}");
            		}
            		else{
            			result = {'success': false,'status':'Internal Server Error',id: ''};
            		}
            	}
